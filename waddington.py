...
sigma_A = 0.2
A = 2.0 + 18.0 * np.exp(-((T - 0.55)/sigma_A)**2)   # keep >0 at ends to preserve two valleys
...
sigma_r_t = 0.24           # width of the mid-time bump
R_base   = 1.2             # baseline height (persists to t=1)
R_peak   = 3.2          # extra mid-time height
tau_on   = 0.2            # onset time scale: smaller -> forms sooner after t=0

onset = 1.0 - np.exp(-T / tau_on)                      # ~0 at t=0, ->1 quickly
central_amp = onset * (R_base + R_peak * np.exp(-((T - 0.55)/sigma_r_t)**2))
...
def A_of_t(t):
    return 2.0 + 18.0 * np.exp(-((t - 0.5)/sigma_A)**2)
...
t_L, x_L = integrate_path(0.5 - 0.005, dt=5e-4,t0=0.022)
t_R, x_R = integrate_path(0.5 + 0.005, dt=5e-4,t0=0.022)

# -------------------- Plot (no rendering call) ------------------------------
fig, ax = plt.subplots(figsize=(4,3), dpi=140)
cf = ax.contourf(X, T, V, levels=60, cmap='terrain')
ax.contour(X, T, V, levels=12, colors='k', linewidths=0.35, alpha=0.5)

ax.plot(x_L, t_L, lw=2.2, color='w',label='→ left fate (x→0.25)')
ax.plot(x_R, t_R, lw=2.2, color='w',label='→ right fate (x→0.75)')

ax.scatter([0.5], [0.022], s=60, edgecolor='k', facecolor='white', zorder=5)
ax.scatter([x_L[-1]], [0.978], s=60, edgecolor='k', facecolor='white', zorder=5)
ax.scatter([x_R[-1]], [0.978], s=60, edgecolor='k', facecolor='white', zorder=5)

ax.text(0.5,0.08,'stem cell',va='center',ha='center')
ax.text(0.5,0.95,'mature cells',va='center',ha='center')
...
ax.arrow(-0.03,0,0,1,clip_on=False,length_includes_head=True,head_width=0.03,head_length=0.03,color='k')
