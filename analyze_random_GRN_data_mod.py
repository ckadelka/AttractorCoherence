...
if __name__ == '__main__':
    ...
    n_points = 1000
    auc_attractor_coherencess = np.array([[utils.get_auc(basin_sizess[i][j],attractor_coherencess[i][j],n_points) for j,k in enumerate([0,1,n])] for i,n in enumerate(ns)])
    auc_basin_coherencess = np.array([[utils.get_auc(basin_sizess[i][j],basin_coherencess[i][j],n_points) for j,k in enumerate([0,1,n])] for i,n in enumerate(ns)])
    ...
    for index_n,n in enumerate(ns):
        for j,k in enumerate([0,1,n]):
            ...
            ax[index_n,j].plot(basin_sizess[index_n][j][indices]/2**N,0.5*np.array(utils.rolling_mean(basin_coherencess[index_n][j][indices],window_size)) + 0.5*np.array(utils.rolling_mean(basin_coherencess[index_n][j][indices][::-1],window_size)[::-1]),color=cmap(0),label='basin coherence',lw=2)
            ax[index_n,j].plot(basin_sizess[index_n][j][indices]/2**N,0.5*np.array(utils.rolling_mean(attractor_coherencess[index_n][j][indices],window_size)) + 0.5*np.array(utils.rolling_mean(attractor_coherencess[index_n][j][indices][::-1],window_size)[::-1]),color=cmap(1),label='attractor coherence',lw=1)
    
            
            if OPTION=='lines':
                ax[index_n,j].plot(basin_sizess[index_n][j][indices]/2**N,utils.running_max(basin_coherencess[index_n][j][indices]),color=cmap(0),ls=':',lw=1)
                ax[index_n,j].plot(basin_sizess[index_n][j][indices]/2**N,utils.running_min(basin_coherencess[index_n][j][indices]),color=cmap(0),ls=':',lw=1)
                ax[index_n,j].plot(basin_sizess[index_n][j][indices]/2**N,utils.running_max(attractor_coherencess[index_n][j][indices]),color=cmap(1),ls=':',lw=1)
                ax[index_n,j].plot(basin_sizess[index_n][j][indices]/2**N,utils.running_min(attractor_coherencess[index_n][j][indices]),color=cmap(1),ls=':',lw=1)
            elif OPTION=='area':
                ax[index_n,j].fill_between(basin_sizess[index_n][j][indices]/2**N,utils.running_min(attractor_coherencess[index_n][j][indices]),utils.running_max(attractor_coherencess[index_n][j][indices]),color=cmap(1),alpha=0.1)
                ax[index_n,j].fill_between(basin_sizess[index_n][j][indices]/2**N,utils.running_min(basin_coherencess[index_n][j][indices]),utils.running_max(basin_coherencess[index_n][j][indices]),color=cmap(0),alpha=0.1)
    ...
    plt.savefig(f'figs/basin_vs_attractor_coherence_random_{N}_{num_simulations}_{OPTION}_{SHOW_AUC_TEXT}.pdf',bbox_inches='tight')
    ...
    df.to_excel(f'figs/basin_vs_attractor_coherence_N{N}_exactdepth{exact_depth}_table.xlsx')
    ...
    n_points = 1000
    auc_attractor_coherencess = ([[utils.get_auc(basin_sizess[i][j],attractor_coherencess[i][j],n_points) for j,layer_structure in enumerate(layer_structuress[i])] for i,n in enumerate(ns)])
    auc_basin_coherencess = ([[utils.get_auc(basin_sizess[i][j],basin_coherencess[i][j],n_points) for j,layer_structure in enumerate(layer_structuress[i])] for i,n in enumerate(ns)])
    ...
    for n in ns:
        ...
        for i in range(nrow):
            for j in range(ncol):
                ...
                if gap_size > max_gap_size:
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices[:index_gap]]/2**N,utils.rolling_mean(basin_coherencess[index_n][ncol*i+j][indices[:index_gap]],window_size),color=cmap(0),label='basin coherence',lw=2)
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices[:index_gap]]/2**N,utils.rolling_mean(attractor_coherencess[index_n][ncol*i+j][indices[:index_gap]],window_size),color=cmap(1),label='attractor coherence',lw=1)
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices[index_gap:]]/2**N,utils.rolling_mean(basin_coherencess[index_n][ncol*i+j][indices[index_gap:]],window_size),color=cmap(0),lw=2)
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices[index_gap:]]/2**N,utils.rolling_mean(attractor_coherencess[index_n][ncol*i+j][indices[index_gap:]],window_size),color=cmap(1),lw=1)
                else:
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices]/2**N,0.5*np.array(utils.rolling_mean(basin_coherencess[index_n][ncol*i+j][indices],window_size)) + 0.5*np.array(utils.rolling_mean(basin_coherencess[index_n][ncol*i+j][indices][::-1],window_size)[::-1]),color=cmap(0),label='basin coherence',lw=2)
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices]/2**N,0.5*np.array(utils.rolling_mean(attractor_coherencess[index_n][ncol*i+j][indices],window_size)) + 0.5*np.array(utils.rolling_mean(attractor_coherencess[index_n][ncol*i+j][indices][::-1],window_size)[::-1]),color=cmap(1),label='attractor coherence',lw=1)        
                if OPTION=='lines':
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices]/2**N,utils.running_max(basin_coherencess[index_n][ncol*i+j][indices]),color=cmap(0),ls=':',lw=1)
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices]/2**N,utils.running_min(basin_coherencess[index_n][ncol*i+j][indices]),color=cmap(0),ls=':',lw=1)
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices]/2**N,utils.running_max(attractor_coherencess[index_n][ncol*i+j][indices]),color=cmap(1),ls=':',lw=1)
                    ax[idx].plot(basin_sizess[index_n][ncol*i+j][indices]/2**N,utils.running_min(attractor_coherencess[index_n][ncol*i+j][indices]),color=cmap(1),ls=':',lw=1)
                elif OPTION=='area':
                    ax[idx].fill_between(basin_sizess[index_n][ncol*i+j][indices]/2**N,utils.running_min(attractor_coherencess[index_n][ncol*i+j][indices]),utils.running_max(attractor_coherencess[index_n][ncol*i+j][indices]),color=cmap(1),alpha=0.1)
                    ax[idx].fill_between(basin_sizess[index_n][ncol*i+j][indices]/2**N,utils.running_min(basin_coherencess[index_n][ncol*i+j][indices]),utils.running_max(basin_coherencess[index_n][ncol*i+j][indices]),color=cmap(0),alpha=0.1)
        ...
        plt.subplots_adjust(hspace=0.3)
        plt.savefig(f'figs/basin_vs_attractor_coherence_randomlayers_degree{n}_{N}_{num_simulations}_{window_size}_{OPTION}_{SHOW_AUC_TEXT}.pdf',bbox_inches='tight')
        ...
        plt.subplots_adjust(hspace=0.3)
        plt.savefig(f'figs/basinsize_distribution_randomlayers_degree{n}_{N}_{num_simulations}.pdf',bbox_inches='tight')
    ...
    df = pd.DataFrame(np.c_[list(map(str,all_layer_structures)),aucs_basin,aucs_attractor,difference_aucs,
                       ...
                       boolforge.utils.flatten(avg_n_attractors),
                       np.array([boolforge.utils.flatten(avg_n_m_cycles[m]) for m in range(max_length)]).T,
                       boolforge.utils.flatten(avg_n_long_cycles)],
    ...
    writer = pd.ExcelWriter(f'figs/basin_vs_attractor_coherence_layers_ns{ns}.xlsx',
                            engine='xlsxwriter',
    ...
    plt.savefig('figs/basin_vs_attractor_coherence_layers_bias_avgsens.pdf',bbox_inches='tight')
    ...
    colors = [cmap(1.),cmap(0.88),cmap(0.2),cmap(0.1),cmap(0.4)]
    max_gap_size=0.02
    xs_per_subfigure = [[] for _ in range(ncol*nrow)]
    ys_per_subfigure = [[] for _ in range(ncol*nrow)]
    for n in ns_to_plot:
        ii = ns.index(n)
        input_redundancies = []
        average_degrees = []
        ...
        for iii,(values_y,name_y) in enumerate(zip([aucs_basin,aucs_attractor,difference_aucs],['AUC basin coherence\n(AUC:BC)','AUC attractor coherence\n(AUC:AC)','Difference in AUC values\n('+r'$\Delta$AUC)'])):
            for jjj,(values_x,name_x) in enumerate(zip([avg_sens,standardized_biases],['Average sensitivity','Standardized bias'])):
                ...
                xs_per_subfigure[counter].extend(list(values_x))
                ys_per_subfigure[counter].extend(list(values_y))
                if iii == 2:
                    ax[iii,jjj].set_xlabel(name_x)
                if jjj == 0:
                    ax[iii,jjj].set_ylabel(name_y)
    ...
    for iii,(values_y,name_y) in enumerate(zip([aucs_basin,aucs_attractor,difference_aucs],['AUC basin coherence\n(AUC:BC)','AUC attractor coherence\n(AUC:AC)','Difference in AUC values\n('+r'$\Delta$AUC)'])):
        for jjj,(values_x,name_x) in enumerate(zip([avg_sens,standardized_biases],['Average sensitivity','Standardized bias'])):
            spearman = stats.spearmanr(xs_per_subfigure[counter],ys_per_subfigure[counter])[0]
            y1,y2 = ax[iii,jjj].get_ylim()
            x1,x2 = ax[iii,jjj].get_xlim()
            ax[iii,jjj].text(x1+0.4*(x2-x1),y1 + 0.1*(y2-y1),r'$\rho = $'+f"{np.round(spearman,3)}",va='center',ha='center')
            counter+=1

    plt.savefig(f'figs/basin_vs_attractor_coherence_layers_3_by_{ncol}_ns{ns}.pdf',bbox_inches='tight')
